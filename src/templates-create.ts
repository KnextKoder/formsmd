import { addReservedClass } from "./attrs-parse";
import { parseDivs, parseBindSpans } from "./div-span-parse";
...
export interface ParsedBodyTemplate {
	template: string;
	settings: Record<string, any>;
}

export interface ParsedContentTemplate {
	template: string;
 ...
 */
export function createStyles(settings: Record<string, any>): string {
	const styleBlocks = [];
 ...
 */
export function createBodyTemplate(
	settings: Record<string, any>,
): ParsedBodyTemplate {
	marked.use({
		renderer: renderer,
	...
	if (settings.brand !== undefined) {
		let brand = marked.parseInline(settings.brand) as string;
		...
		if (settings["brand-alt-scheme"] !== undefined) {
			let brandAltScheme = marked.parseInline(
				settings["brand-alt-scheme"],
			) as string;
	...
	if (settings.cta !== undefined) {
		let cta = marked.parseInline(settings.cta) as string;
...
export function createContentTemplate(
	...
	template = template.replace(
		...
		function (_match: string, content: string): string {
			if (windowAndSanitize && typeof window !== "undefined") {
