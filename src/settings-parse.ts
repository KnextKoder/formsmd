 ...
 */
export function parseSettings(template: string): ParsedSettings {
	// Settings references
	const settingsRef: Record<string, { pattern?: RegExp; accepted: string }> = {
		"accent": {
	...
	for (let [key, value] of Object.entries(userSettings)) {
		...
		if (
			...
			const pattern = settingsRef[key].pattern;
