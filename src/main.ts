...
import { parseDataBlocks } from "./data-blocks-parse";
import { isNumeric } from "./helpers";
import { renderer } from "./marked-renderer";
import { getDefaultSettings, parseSettings } from "./settings-parse";
import { parseSpreadsheetData } from "./spreadsheet-data-parse";
import {
	createStyles,
...
export class Formsmd {
	 ...
	 */
	constructor(template: string, container: any, options?: Record<string, any>) {
	...
	saveFieldValue = (name, value) => {
		...
		let savedFormData: any = localStorage.getItem(localStorageKey) || "{}";
	...
	setFormDataToState = () => {
		...
		instance.container
			...
			.forEach((elem) => {
				const name = elem.getAttribute("name");
				let value: any = instance.getRadioCheckboxValue(
					name,
	...
	numChoiceFieldOnInput = (e) => {
		...
		const name = (e.target as any).getAttribute("name");
		const value = parseInt(
			instance.getRadioCheckboxValue(
				name,
				"fmd-form-num-check-input",
				"radio",
			) as string,
	...
	postFormData = (postCondition, end) => {
		...
		instance.container
			...
			.forEach((input: any) => {
				const name = input.getAttribute("name");
				formData.append(name, String(input.checked));
			});

		// Set the extra fields
		formData.append("_end", end ? String(end) : "");
		...
		return processFiles().then(() => {
			...
			if (instance.options.recaptcha.siteKey) {
				return instance.executeRecaptcha().then((token) => {
					formData.append("_captcha", token as any);
	...
	getSlideTransitionDuration = (): number => {
		...
		// Otherwise, calculate duration from CSS, save on root element, and return
			...
			const duration = Number(durationStr.slice(0, -2));
			rootElem.setAttribute(
				"data-fmd-slide-transition-duration",
				String(duration),
			);
			return duration;
		}
